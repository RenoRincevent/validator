...
{
	RV32G_access_t access = RV32G_access_read;
	uint8_t *ptr8;
	uint16_t *ptr16;
	uint32_t *ptr32;
	uint64_t *ptr64;

	if (type_access == access)
	{
...
void debug_callback(RV32G_address_t addr, int size, void *data, unsigned int type_access)
{
	...
	uint32_t *ptr32;
	uint64_t *ptr64;
	RV32G_memory_t *mem = RV32G_get_memory(platform, RV32G_MAIN_MEMORY);
	...
	if (type_access == access) {
		...
		case 8:
			ptr64 = (uint64_t *)data;
			printf("debug-mem(%08X)W, [%08X] = %016lX\n", gliss_pc, addr, *ptr64);
			break;
		default:
			ptr8 = (uint8_t *)data;
			int i;
