...
void compare_regs_this_instruction(char *drive_gdb_reply_buffer, RV32G_state_t *real_state, RV32G_inst_t *instr, int instr_count)
{
	int i;
		...
		if (reg_infos[i].relative)
		{
			if (instr_count && ((reg_infos[i].gliss - reg_infos[i].gliss_last) != (reg_infos[i].gdb - reg_infos[i].gdb_last)))
			{
				if (reg_infos[i].size == 64)
					fprintf(stdout, "\n\nAfter 0x%08x (instruction #%i), register %s has not changed by the same amount: gdb %016lX\tgliss %016lX\n",
						gdb_pc, instr_count, reg_infos[i].name, reg_infos[i].gdb - reg_infos[i].gdb_last, reg_infos[i].gliss - reg_infos[i].gliss_last);
				else
					fprintf(stdout, "\n\nAfter 0x%08x (instruction #%i), register %s has not changed by the same amount: gdb %08lX\tgliss %08lX\n",
						gdb_pc, instr_count, reg_infos[i].name, reg_infos[i].gdb - reg_infos[i].gdb_last, reg_infos[i].gliss - reg_infos[i].gliss_last);
				dump_regs();
